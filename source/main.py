...
def round_entry():
    ...
    return render_template("round_entry.html", settings=g.settings, courses=g.courses, today=today, no_courses=no_courses)
...
def main():
    ...
    chrome_proc = None
    if chrome:
        chrome_proc = subprocess.Popen([chrome, f"--app={url}", "--start-maximized"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, env=os.environ.copy())
    else:
        webbrowser.open(url)

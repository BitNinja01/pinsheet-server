 ...
 *
 * Each player gets a stable mascot and nickname derived from their user id
 * (same person -> same mascot every visit). A roaming mascot also randomly
 * "spawns" over the board for a bit of fun, then wanders off.
 *
 * No dependencies. Activates on any element carrying data-mascot-seed.
 */
(function () {
    "use strict";

    // Golf-pun nicknames. The leader (data-mascot-leader="1") draws from the
    // flattering pool; everyone else gets the cheeky one.
    var NICKS = [
    ...
    function init() {
        var seeds = [];
        document.querySelectorAll("[data-mascot-seed]").forEach(function (el) {
            var seedStr = "pinsheet:" + el.getAttribute("data-mascot-seed");
            var isLeader = el.getAttribute("data-mascot-leader") === "1";
            ...
            if (nickEl && !nickEl.textContent.trim()) {
                nickEl.textContent = nicknameFor(seedStr, isLeader);
            }
        });
    ...
    function startRoamer(seeds) {
        var board = document.querySelector("[data-mascot-stage]") || document.body;

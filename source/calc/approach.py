...
def calc_scoring_by_fairway(rounds: list[RoundData], courses: dict[str, CourseData]) -> dict:
    ...
    for r in rounds:
        ...
        for hole_num, h in r.holes.items():
            par = holes.get(str(hole_num), HoleDef()).par
            if par == 3 or not h.gross or h.fairway == "N":
                continue
            diff = h.gross - par
            fw = h.fairway
            if not fw or fw == "H":
...
def calc_gir_by_par_type(rounds: list[RoundData], courses: dict[str, CourseData]) -> dict:
    ...
    for r in rounds:
        ...
        for hole_num, h in r.holes.items():
            par = holes.get(str(hole_num), HoleDef()).par
            if par not in totals or h.gir == "N":
                continue
            totals[par] += 1
            if not h.gir or h.gir == "H":
                hits[par] += 1
...
def calc_gir_from_fairway_vs_rough(rounds: list[RoundData], courses: dict[str, CourseData]) -> dict:
    ...
    for r in rounds:
        ...
        for hole_num, h in r.holes.items():
            par = holes.get(str(hole_num), HoleDef()).par
            if par == 3 or h.fairway == "N" or h.gir == "N":
                continue
            fw = h.fairway
            ...
            if not fw or fw == "H":
                ...
                if gir_hit:
                    fw_hits += 1
            elif fw in {"L", "OBL", "R", "OBR", "S", "LO", "OBS", "OBLO"}:
                rough_total += 1
                if gir_hit:
                    rough_hits += 1
    return {
...
def calc_scoring_by_gir(rounds: list[RoundData], courses: dict[str, CourseData]) -> dict:
    ...
    for r in rounds:
        ...
        for hole_num, h in r.holes.items():
            par = holes.get(str(hole_num), HoleDef()).par
            if not par or not h.gross or h.gir == "N":
                continue
            diff = h.gross - par
            if not h.gir or h.gir == "H":
                hit_diffs.append(diff)
...
def calc_scramble_by_miss_direction(rounds: list[RoundData], courses: dict[str, CourseData]) -> dict:
    ...
    for r in rounds:
        ...
        for hole_num, h in r.holes.items():
            gir = h.gir
            if not gir or gir in ("H", "N") or gir not in _map or not h.gross:
                continue
            par = holes.get(str(hole_num), HoleDef()).par
            if not par:
                continue
...
def calc_scramble_by_par_type(rounds: list[RoundData], courses: dict[str, CourseData]) -> dict:
    ...
    for r in rounds:
        ...
        holes = course.holes if course else {}
        for hole_num, h in r.holes.items():
            if not h.gir or h.gir in ("H", "N") or not h.gross:
                continue
            par = holes.get(str(hole_num), HoleDef()).par
